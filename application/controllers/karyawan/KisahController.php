...
class KisahController extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_karyawan_kisah', 'mkisah');
	...
	public function index()
	{
		...
		$data['vitamin']                  = 'karyawan_kisah_vitamin';
		$data['status_filter']            = NULL;
		$data['karyawan_karyawan_filter'] = NULL;
		...
		if($this->input->get('status_filter') != NULL){
			...
			$karyawan_karyawan_filter = 'karyawan';
			$tipe_filter              = $this->input->get('tipe_filter');
			$kata_filter              = $this->input->get('kata_filter');

			$config['base_url']            = site_url('karyawan/kisah/index');
			$config['total_rows']          = $this->mkisah->count($status_filter);
			...
			$data['arrs']          = $this->mkisah->get_data($limit, $offset, $status_filter);
		}
	...
	public function create()
	{
		...
		$data['vitamin'] = 'karyawan_kisah_form_vitamin';
	...
	public function show($id)
	{
		...
		if($arrs->num_rows() == 0){
			...
			$code         = 200;
			$data['arrs'] = $arrs;
			$result       = $this->load->view('karyawan/kisah/modal', $data, TRUE);
	...
	public function edit($id)
	{
		...
		$data['vitamin'] = 'karyawan_kisah_form_edit_vitamin';
...
/* Location: ./application/controllers/karyawan/KisahController.php */