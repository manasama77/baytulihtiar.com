...
class AnggotaController extends CI_Controller {
	...
	{
		parent::__construct();
		$this->load->model('M_backend_anggota', 'manggota');
	...
	public function index()
	{
		...
		$data['vitamin'] = 'admin_anggota_vitamin';
		...
		$config['base_url']            = site_url('backend/anggota/index');
		$config['total_rows']          = $this->mcore->count('anggota', NULL);
		...
		$data['arrs'] = $this->mcore->get('anggota', '*', NULL, 'id', 'ASC', $limit, $offset);
	...
	{
		$arrs = $this->mcore->get('anggota', '*', ['id' => $id], NULL, 'ASC', NULL, NULL);
		...
		if($arrs->num_rows() == 0){
			...
			$code         = 200;
			$data['arrs'] = $arrs;
			$result       = $this->load->view('admin/anggota/modal', $data, TRUE);

			// DEBUG PURPOSE ONLY
			// $data['arrs'] = $arrs;
			// return $this->load->view('anggota/anggota/modal', $data, FALSE);
		}

		echo json_encode([
			'code'  => $code,
			'judul' => 'Edit Anggota - '.$arrs->row()->nama,
			'data'  => $result
		]);
		exit;
	}
	...
	public function update()
	{
		...
		$majlis = $this->input->post('majlis_edit');
	...
	public function flag($flag, $id)
	{
		$url = $_SERVER['HTTP_REFERER'];
		if($flag == 1){
			$flag_msg = 'Aktifkan anggota berhasil';
			$object = ['flag_aktif' => 'aktif'];
		}elseif($flag == 0){
			$flag_msg = 'Non Aktifkan anggota berhasil';
			$object = ['flag_aktif' => 'tidak aktif'];
		}

		$exec = $this->mcore->update('anggota', $object, ['id' => $id]);
	...
	public function reset()
	{
		...
		$exec     = $this->mcore->update('anggota', ['password' => $password], ['id' => $id]);
...
/* Location: ./application/controllers/backend/AnggotaController.php */