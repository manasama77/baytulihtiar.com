...
<script>
	$(document).ready(function(){
		...
		$('#nik').on('change', function(){
			$.blockUI();
			$.get('<?=base_url();?>backend/karyawan/check/'+$(this).val(), null , 
		...
		$('#form').validate({
			...
			submitHandler: function( form ) {
				console.log(form);
				$.ajax({
					url         : '<?=site_url('backend/karyawan/store');?>',
					method      : 'POST',
...
<script>
	...
	{
		$.ajax({
			url         : `<?=site_url();?>backend/karyawan/edit/${id}`,
			method      : 'GET',
			dataType    : 'json',
			beforeSend  : function(){
	...
	function showModalReset(id)
	{
		...
		$('#form-reset').validate({
			...
			submitHandler: function( form ) {
				console.log(form);
				$.ajax({
					url         : '<?=site_url('backend/karyawan/reset');?>',
					method      : 'POST',
					data        : new FormData(form),
					dataType    : 'JSON',
					processData : false,
	...
	function confirmDestroy(id)
	{
		...
		let beneran = confirm("Delete Karyawan ?");
		let href = `<?=site_url();?>backend/karyawan/destroy/${id}`;
	...
	function confirmAktivasi(id, flag)
	{
		...
		if(flag == '1'){
			beneran = confirm("Aktifkan Karyawan ?");
		}else{
			beneran = confirm("Non Aktifkan Karyawan ?");
		}

		let href = `<?=site_url();?>backend/karyawan/flag/${flag}/${id}`;
