...
<script>
	...
	{
		$.ajax({
			url         : `<?=site_url();?>karyawan/kisah/show/${id}`,
			method      : 'GET',
			dataType    : 'json',
			beforeSend  : function(){
	...
	function confirmDestroy(id)
	{
		...
		let beneran = confirm("Delete Kisah ?");
		let href = `<?=site_url();?>karyawan/kisah/destroy/${id}`;
	...
	function confirmAktivasi(id, flag)
	{
		...
		if(flag == '1'){
			beneran = confirm("Aktifkan Kisah ?");
		}else{
			beneran = confirm("Non Aktifkan Kisah ?");
		}

		let href = `<?=site_url();?>karyawan/kisah/flag/${flag}/${id}`;
